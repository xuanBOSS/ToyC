...
private:
    ...
    // 循环优化相关
    std::map<std::string, std::string> loopVarToRegMap;  // 循环变量到专用寄存器的映射
    std::set<std::string> loopInvariants;               // 循环不变量集合
    
    // 指令缓存 - 避免频繁的字符串操作
    std::map<std::string, std::string> instrCache;
...
private:
    ...
    // 循环优化相关方法
    bool isSimpleCountingLoop(const std::shared_ptr<WhileStmt>& stmt);    // 判断是否为简单计数循环
    bool hasIncrementPattern(const std::shared_ptr<Stmt>& stmt, const std::string& varName); // 检测是否包含变量自增
    void processOptimizedLoop(const std::shared_ptr<WhileStmt>& stmt);    // 处理优化的循环
    void processLoopBodyWithoutIncrement(const std::shared_ptr<Stmt>& body, const std::string& loopVar); // 处理循环体
    void optimizeLoopInvariants(const std::shared_ptr<WhileStmt>& stmt, const std::string& loopVar); // 优化循环不变量
    bool isLoopInvariant(const std::shared_ptr<Expr>& expr, const std::string& loopVar); // 判断表达式是否是循环不变的
    void collectExpressions(const std::shared_ptr<Stmt>& stmt, std::vector<std::shared_ptr<Expr>>& expressions); // 收集表达式
